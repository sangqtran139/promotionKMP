...
final class MyPromotionViewController: PRMBaseViewController<MyPromotionViewModel> {
    ...
    override func setupAccessibility() {
        ...
        self.promotionsTableview.accessibilityIdentifier = PRMAccessibilityID.MyPromotion.table
        self.expiringPromotionTabView.accessibilityIdentifier = PRMAccessibilityID.MyPromotion.tabExpiring
        self.allPromotionTabView.accessibilityIdentifier = PRMAccessibilityID.MyPromotion.tabAll
        self.headerView.accessibilityIdentifier = PRMAccessibilityID.MyPromotion.header

        // Hai nút này CHỈ có icon → VoiceOver không có gì để đọc nếu thiếu nhãn. Đặt từ tầng SDK chứ
        // không đặt trong `PRMPromotionUI`: chuỗi hiển thị tập trung ở `PromotionUIStrings`, package
        // dùng chung không giữ tiếng Việt.
        //
        // `isAccessibilityElement = true` là bắt buộc: `PRMTapableView` kế thừa `UIControl` trần chứ
        // không phải `UIButton`, mà `UIControl` trần KHÔNG tự là phần tử accessibility. Thiếu dòng
        // này thì nhãn có đặt cũng không ai đọc được, và XCUITest không thấy nút.
        for (button, label, identifier) in [
            (self.headerView.backButton,
             PromotionUIStrings.A11y.backButton,
             PRMAccessibilityID.MyPromotion.backButton),
            (self.headerView.searchButton,
             PromotionUIStrings.A11y.searchButton,
             PRMAccessibilityID.MyPromotion.searchButton)
        ] {
    ...
    private func configHeaderView() {
        self.headerView.titleText = PromotionUIStrings.myPromotions
...
extension MyPromotionViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Định danh gắn theo indexPath chứ không phải một tên chung: XCUITest cần trỏ vào ĐÚNG một
        // hàng ("hàng thứ 3 phải là voucher hết hạn"), không phải "có tồn tại một cell nào đó".
        // Đặt ở đây vì cell được tái sử dụng — trong `awakeFromNib` thì chỉ số sẽ sai sau lần cuộn đầu.
        cell.accessibilityIdentifier = PRMAccessibilityID.Cell.indexed(
            PRMAccessibilityID.Cell.myPromotion, indexPath
        )
        return cell
    }
}
