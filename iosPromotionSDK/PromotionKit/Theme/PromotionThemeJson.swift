//
//  PromotionThemeJson.swift
//  PromotionSDK
//
//  Serialize PromotionSDKTheme ↔ JSON. **Một định dạng dùng chung với Android**
//  (`PromotionThemeJson.kt`): key nhóm `button`/`searchBar`…, màu hex `#AARRGGBB`.
//  Đối tác ship một file theme cho cả hai nền tảng.
//

import UIKit

/// Serialize `PromotionSDKTheme` ↔ JSON. **Một định dạng dùng chung với Android** — cùng tên type
/// (`PromotionThemeJson`), cùng hàm `toJson` / `fromJson`. Đối ứng `PromotionThemeJson.kt`.
public enum PromotionThemeJson {

    /// Serialize theme → JSON (hex `#AARRGGBB`, key nhóm `button`/`searchBar`…). "{}" nếu encode lỗi.
    public static func toJson(_ theme: PromotionSDKTheme) -> String {
    ...
    public static func fromJson(_ json: String) -> PromotionSDKTheme? {
