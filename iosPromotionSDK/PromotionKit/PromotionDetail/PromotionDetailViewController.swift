...
final class PromotionDetailViewController: PRMBaseViewController<PromotionDetailViewModel> {
    ...
    override func setupAccessibility() {
        ...
        self.voucherCardView.accessibilityIdentifier = PRMAccessibilityID.PromotionDetail.voucherCard
        self.underlinedSegmentControlView.accessibilityIdentifier = PRMAccessibilityID.PromotionDetail.segmentControl
        self.applyButton.accessibilityIdentifier = PRMAccessibilityID.PromotionDetail.applyButton

        // Banner MANG thông tin (ảnh nhận diện của ưu đãi) nên là phần tử đọc được, khác với ảnh
        // trang trí trong cell — chỗ đó để VoiceOver bỏ qua, xem `MyPromotionCell`.
        self.bannerImageView.accessibilityIdentifier = PRMAccessibilityID.PromotionDetail.banner
        self.bannerImageView.isAccessibilityElement = true
        self.bannerImageView.accessibilityTraits = .image
        self.bannerImageView.accessibilityLabel = PromotionUIStrings.A11y.promotionBanner
    }
    ...
    /// Đối ứng `javaScriptEnabled = false` ở `PrmContentDetailOfferFragment` bên Android — sửa một
    /// bên thì sửa cả hai.
