...
final class ChoosePromotionViewController: PRMBaseViewController<ChoosePromotionViewModel> {
    ...
    /// `onSettled` của `PRMOfferWidget.applySelectedOffers` bên Android (cũng đã đổi sang
    ...
    var onApplyVoucher: (([EligibleOffer], @escaping (OfferWidgetApplyOutcome) -> Void) -> Void)?
    ...
    override func setupAccessibility() {
        ...
        self.searchTextField.accessibilityIdentifier = PRMAccessibilityID.ChoosePromotion.searchField
        self.promotionsTableView.accessibilityIdentifier = PRMAccessibilityID.ChoosePromotion.table
        self.totalVoucherView.accessibilityIdentifier = PRMAccessibilityID.ChoosePromotion.selectedSummary
        self.voucherLabel.accessibilityIdentifier = PRMAccessibilityID.ChoosePromotion.selectedCount

        // CỐ Ý không gộp `totalLabel` + `voucherLabel` thành một phần tử với nhãn dựng sẵn: số
        // voucher đổi theo từng lần chọn, mà `setupAccessibility()` chỉ chạy một lần lúc dựng màn —
        // nhãn gộp sẽ đứng yên ở giá trị đầu tiên. Để hai label tự đọc thì VoiceOver luôn đọc đúng
        // chữ đang hiển thị.
    }
    ...
    /// Bấm "Áp dụng" → trả offers đang chọn cho widget (`OfferWidgetStore` validate). VM lọc từ store,
    ...
    private func handleApplyOutcome(_ outcome: OfferWidgetApplyOutcome) {
        if let rejected = outcome as? OfferWidgetApplyOutcomeRejected {
        ...
        if let failed = outcome as? OfferWidgetApplyOutcomeFailed {
...
extension ChoosePromotionViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if sectionData.seeMoreState != .none && indexPath.row == sectionData.items.count {
            ...
            cell.configure(
                title: isCollapse ? PromotionUIStrings.collapse : PromotionUIStrings.seeMore,
        ...
        // Kèm cả `section`: màn này có hai section ("Ưu đãi của tôi" / "Ưu đãi khác") nên chỉ số hàng
        // một mình KHÔNG xác định được một ô — xem `PRMAccessibilityID.Cell.indexed`.
        cell.accessibilityIdentifier = PRMAccessibilityID.Cell.indexed(
            PRMAccessibilityID.Cell.choosePromotion, indexPath
        )
        return cell
    }
