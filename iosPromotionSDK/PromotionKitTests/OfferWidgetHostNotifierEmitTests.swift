//
//  OfferWidgetHostNotifierEmitTests.swift
//  PromotionKitTests
//

import XCTest
@testable import PromotionKit
// `OfferWidgetHostEvent` là type Kotlin, PRM import nó bằng `@_implementationOnly` nên nó KHÔNG đi kèm
// `@testable import PromotionKit`. Test phải tự import bridge để dựng được sự kiện.
...
///
/// Rule "khi nào bắn" nằm ở `OfferWidgetHostNotifier` (Kotlin, đã có golden test bên `commonTest`). File
...
@MainActor
final class OfferWidgetHostNotifierEmitTests: XCTestCase {
    ...
    func test_nhieuSuKien_goiDungThuTu() {
        impl.emit([
            OfferWidgetHostEventVoucherApplied(voucherId: "v1"),
            OfferWidgetHostEventVoucherApplied(voucherId: "v2"),
            OfferWidgetHostEventVoucherApplied(voucherId: "v3"),
        ])
    ...
    func test_moiSuKienGoiDungMotLan() {
        impl.emit([OfferWidgetHostEventVoucherApplied(voucherId: "v1")])
