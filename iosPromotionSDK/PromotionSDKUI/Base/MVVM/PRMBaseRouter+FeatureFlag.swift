...
extension PRMBaseRouter {
    ...
    /// - Returns: `true` nếu được phép mở. Nếu TẮT → hiện toast lỗi PRM_MOB_021 và trả `false`.
    ///
    /// Quyết định "có được mở không" nằm ở `PromotionFeatureGate` của lõi Kotlin — **dùng chung với
    /// Android**. Ở đây chỉ còn phần riêng của iOS: hiển thị thông báo. Đối ứng bên Android là
    ...
    func canOpenVoucherDetail() -> Bool {
        ...
        if let container = viewController?.view ?? navigator?.topViewController?.view {
            ...
            PromotionToast.show(message, in: container)
        }
        return false
    }
}
