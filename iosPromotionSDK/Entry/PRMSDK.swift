...
//
//  Singleton tĩnh — đối ứng 1:1 `object PRMSDK` bên Android. Toàn bộ điểm vào là hàm/thuộc
...
public final class PRMSDK {
    ...
    private static func requireImpl(_ caller: String) -> PromotionSDKImpl? {
        guard let impl else {
            NSLog("[PRMSDK] %@ bị gọi trước initialize() — bỏ qua. Hãy gọi PRMSDK.initialize(options:) trước.", caller)
            return nil
        }
        return impl
    }

    /// Callback host nhận sự kiện — truyền qua `PRMSDKOptions`. Giữ tới `release` (đối ứng
    /// `private var callback` bên Android).
    private static var callback: PRMSDKCallback?

    // MARK: - Init

    /// Khởi tạo SDK. Đối ứng `PRMSDK.initialize(context, options)` bên Android.
    ///
    ...
    public static func initialize(options: PRMSDKOptions) {
        ...
        // một lần; lần sau chỉ cần initialize lại, theme tự sống lại. Đối ứng PRMSDK.initialize bên Android.
        impl.restoreOrApplyTheme(options.theme)
        wireCallbacks(impl)
    }

    /// Giải phóng SDK. Đối ứng `PRMSDK.release()` bên Android. Gọi khi chưa init là vô hại.
    ...
    /// `true` sau `initialize` và trước `release`. Đối ứng `PRMSDK.isInitialized()` bên Android.
    ...
    public static func getCallback() -> PRMSDKCallback? { callback }

    // MARK: - Headless API

    /// Bề mặt headless cho host tự dựng UI (lấy voucher, validate, tạo redemption). Phải gọi
    /// `initialize` trước. Không phải use case — xem `PRMSDKApi`. Đối ứng `PRMSDK.api` Android.
    ///
    ...
    public static var api: PRMSDKApi {
        guard let impl else {
            preconditionFailure("PRMSDK.initialize() phải được gọi trước khi truy cập api.")
        }
        return impl.makeApi()
    }
    ...
    /// Session đã truyền lúc `initialize`. `nil` khi chưa `initialize`. Đối ứng `PRMSDK.session`.
    public static var session: PRMSessionConfig? { impl?.context.session }
    ...
    /// request, nên gọi trước khi mở màn hoặc gọi API là đủ. Đối ứng `PRMSDK.updateContext` Android.
    ///
    ...
    /// quay về mặc định SDK. Đối ứng `PRMSDK.configure(theme:)` bên Android.
    ///
    /// Lưu ý: theme nên cấu hình **một lần** lúc khởi tạo. Các view đã render có thể chỉ cập nhật khi
    /// được dựng lại (rebind / đẩy màn mới).
    public static func configure(theme: PRMSDKTheme?) {
        impl?.applyAndPersistTheme(theme)
    }

    /// Theme đang áp (`nil` nếu đang dùng mặc định). Đối ứng `PRMSDK.currentTheme()` bên Android.
    public static func currentTheme() -> PRMSDKTheme? {
        impl?.currentTheme()
    }
    ...
    public static func createEndowView(from viewController: UIViewController,
                                       orderId: String?,
                                       orderValue: String?,
                                       orderItems: [PRMOrderItem]) -> UIView {
