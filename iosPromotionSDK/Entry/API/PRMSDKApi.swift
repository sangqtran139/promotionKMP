...
///
/// Việc nó làm, và chỉ một việc: đổi model Kotlin sang DTO Swift (`PRMVoucher`,
/// `PRMEligibleOffer`, …), và đổi `PromotionResult` sang `PromotionApiResult`.
///
/// **Song ánh với `PRMSDKApi.kt` bên Android**: cùng 5 hàm, cùng tên tham số, cùng DTO, cùng
...
public final class PRMSDKApi {
    ...
    public func getVouchers(
        ...
        page: Int = 0,
        size: Int = 10,
        completion: @escaping (PromotionApiResult<PRMVoucherPage>) -> Void
    ) {
        ...
        handle(
            completion,
            onEmpty: { .success(PRMVoucherPage(vouchers: [], isLastPage: true)) },
    ...
    public func findEligible(
        ...
        otherPage: Int = 0,
        otherSize: Int = 10,
        completion: @escaping (PromotionApiResult<PRMEligibleResult>) -> Void
    ) {
        ...
        handle(
            completion,
            onEmpty: {
                .success(PRMEligibleResult(myOffers: [], otherOffers: [], myIsLastPage: true, otherIsLastPage: true))
            },
    ...
    public func getVoucherDetail(
        voucherId: String,
        serviceCode: String? = nil,
        completion: @escaping (PromotionApiResult<PRMVoucherDetail>) -> Void
    ) {
        let useCases = self.useCases
        handle(
            completion,
    ...
    public func validateDiscounts(
        ...
        completion: @escaping (PromotionApiResult<PRMValidationResult>) -> Void
    ) {
    ...
    public func createRedemption(
        ...
        voucherIds: [String],
        objectType: String = "CAMPAIGN",
        completion: @escaping (PromotionApiResult<PRMRedemptionResult>) -> Void
    ) {
            ...
            map: { (result: CreateRedemptionResult) in
                PRMRedemptionResult(
                    ...
                    validationErrors: result.validationErrors.map {
                        PRMRedemptionError(code: $0.code, message: $0.message)
                    }
                )
            },
    ...
    ///   Giữ đúng hành vi của `PRMSDKApi.kt`.
    ...
    private static func toSdkError(_ failure: PromotionResultFailure) -> PRMSDKError {
    ...
    private static func bridgeMismatch(_ result: any PromotionResult) -> NSError {
        NSError(
            ...
            userInfo: [NSLocalizedDescriptionKey:
                "PromotionResult không rõ biến thể: \(type(of: result)). Kiểm tra PRMSDKApi.handle."]
        )
    }

    // MARK: - Lõi → DTO public

    private static func toVoucher(_ model: VoucherItem) -> PRMVoucher {
        PRMVoucher(
            id: model.voucherId,
    ...
    private static func toVoucherDetail(_ model: VoucherDetail) -> PRMVoucherDetail {
        PRMVoucherDetail(
            id: model.voucherId,
            merchantName: model.merchantName ?? "",
            title: model.title ?? "",
    ...
    private static func toOffer(_ model: EligibleOffer) -> PRMEligibleOffer {
        PRMEligibleOffer(
            id: model.id,
