...
final class MyPromotionViewController: BaseViewController<MyPromotionViewModel> {
    ...
    // MARK: - UI Components
    @IBOutlet private weak var promotionsTableview: RefreshTableView!
    ...
    private var tabShimmerChips: [VDSShimmerView] = []
    ...
    private lazy var emptyView: PromotionSearchNoResultView = {
        ...
        view.thumbnailImage = UIImage.sdk("ic_search_no_result")
    ...
    /// 1 chip skeleton (bo tròn) cho hàng tab — dùng VDSShimmerView để nhấp nháy như card skeleton.
    private func makeTabShimmerChip() -> VDSShimmerView {
        let chip = VDSShimmerView()
    ...
    override func bindViewModel() {
        ...
        output.errorMessage
            .emit(onNext: { [weak self] message in
                guard let self = self else { return }
                VDSConfirmationDialog.showError(message, in: self.view)
            })
            .disposed(by: disposeBag)
    }
