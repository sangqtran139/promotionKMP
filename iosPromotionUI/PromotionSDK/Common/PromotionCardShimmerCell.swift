...
//  Dùng chung cho màn Chọn ưu đãi & Ưu đãi của tôi, qua VDSShimmerReplicatorView (CoreUI).
//

import UIKit
@_implementationOnly import CoreUI

final class PromotionCardShimmerCell: UIView, VDSShimmerReplicatorViewCell {

    /// Chiều cao 1 dòng skeleton (card + khoảng cách dưới).
    static let itemHeight: CGFloat = 96

    private var pieces: [VDSShimmerView] = []
    ...
    private func makePiece(corner: CGFloat) -> VDSShimmerView {
        let v = VDSShimmerView()
    ...
    // MARK: - VDSShimmerReplicatorViewCell
