...
final class ChoosePromotionViewController: BaseViewController<ChoosePromotionViewModel> {
    ...
    // MARK: - UI Components
    @IBOutlet private weak var searchTextField: VDSSearchTextField!
    ...
    private var currentSelectedPromotion: EligibleOffer?

    /// Callback trả về promotion đã chọn về host (tương tự Android onApplyVoucher).
    ...
    private lazy var shimmerView: VDSShimmerReplicatorView = {
        let view = VDSShimmerReplicatorView(
    ...
    private func wireApplyButton() {
        ...
        // Nút "Áp dụng" ở màn này là UIButton thường (không phải VDSButton) → áp button token thủ công.
    ...
    private func applyThemeToApplyButton(_ button: UIButton?) {
        guard let button, let token = VDSThemeRegistry.shared.button() else { return }
    ...
    @objc private func didTapApplyButton() {
        guard let promotion = currentSelectedPromotion else { return }
        onApplyVoucher?(promotion)
    }
...
extension ChoosePromotionViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let tabToken = VDSThemeRegistry.shared.tabUnderline()
