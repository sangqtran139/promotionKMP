...
//  PromotionSDK
//
//  Internal implementation box — keeps all PromotionLogic/RxSwift/PromotionUI types out of
...
final class PromotionSDKImpl: NSObject {
    ...
    func applyTheme(_ theme: PromotionSDKTheme?) {
        ...
        VDSThemeRegistry.shared.configure(config)
    }
    ...
    func showFeatureDisabledDialog(on viewController: UIViewController) {
        ...
        VDSConfirmationDialog.showError(message, in: viewController.view)
    }
            ...
            singleFromKotlin { try await useCase.invoke(request: request) }
                ...
                .subscribe(
                    ...
                    onError: { _ in
                        ...
                        if let vc = vc {
                            VDSConfirmationDialog.showError("Không thể áp dụng ưu đãi lúc này. Vui lòng thử lại.", in: vc.view)
                        }
                    }
                )
