...
extension PRMShimmerSyncTarget {
...
internal extension PRMShimmerView {
    class Animator {
        let shimmerView: PRMShimmerView
        let syncTarget: PRMShimmerSyncTarget
        
        init(shimmerView: PRMShimmerView) {
        ...
        var interpolatedColors: [Any] {
            ...
            var colors: [UIColor] = []
            for index in 0..<baseColors.count - 1 {
                let lengthOfStep = 1.0 / Float(numberOfSteps)
                let newColors = stride(from: 0.0, to: 1.0, by: lengthOfStep).map {
                    baseColors[index].interpolate(with: baseColors[index + 1], degree: CGFloat(simd_smoothstep(0.0, 1.0, $0)))
                }
                colors += newColors
            }
