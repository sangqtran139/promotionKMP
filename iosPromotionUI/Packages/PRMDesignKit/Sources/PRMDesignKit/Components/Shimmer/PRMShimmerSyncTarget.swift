...
/// Shimmer view finds its nearest PRMShimmerSyncTarget in its responder chain and adjusts its shimmering effect according to the sync target's configuration.
public protocol PRMShimmerSyncTarget: UIResponder {
    
    /// The style of the shimmering effect.
    var style: PRMShimmerViewStyle { get }
...
public extension PRMShimmerSyncTarget where Self: UIView {
    var syncTargetView: UIView {
        return self
    }
}

public extension PRMShimmerSyncTarget where Self: UIViewController {
    var syncTargetView: UIView {
        return view
    }
}
