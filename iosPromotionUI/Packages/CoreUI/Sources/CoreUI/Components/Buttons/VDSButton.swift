...
@IBDesignable
public class VDSButton: UIButton {
    
    private var shadowView: VDSShadowView?
    private var gradientView: PassThroughView?
    
    private var imageViewLoading: UIImageView?
    
    private var gradientLayer: CAGradientLayer = CAGradientLayer()
    
    let bundle = Bundle(for: VDSButton.self)
    ...
    /// Size for VDSButton, default is 2
    /// If size is different from VDSButtonSize, default is medium size
    ...
    /// Height from each size of VDSButton
    ...
    private let imageLoading = UIImage.sdk("ic_change_32_white", in: .main)

    /// Theme token host cấu hình (nil = giữ default SDK).
    private var themeToken: VDSButtonThemeToken? { VDSThemeRegistry.shared.button() }
...
extension VDSButton {
    ...
    private func setupButton(isFirstTime: Bool = false) {
        ...
        case VDSButtonSize.small.rawValue:
            ...
            height = smallHeight
        case VDSButtonSize.medium.rawValue:
            ...
            height = mediumHeight
        case VDSButtonSize.large.rawValue:
    ...
    private func getShadowView() -> VDSShadowView {
        switch size {
        case VDSButtonSize.large.rawValue:
            return VDSMediumButtonShadowView()
        case VDSButtonSize.medium.rawValue:
            return VDSMediumButtonShadowView()
        case VDSButtonSize.small.rawValue:
            return VDSSmallButtonShadowView()
        default:
            return VDSLargeButtonShadowView()
        }
    }
}
