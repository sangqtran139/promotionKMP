...
public class PromotionCardView: TapableView {
    ...
    private let checkboxButton: VDSRadioButton = {
        let radio = VDSRadioButton()
        radio.radioTitle = ""
    ...
    private var themeToken: VDSListItemThemeToken? { VDSThemeRegistry.shared.listItem() }
    ...
    private func setupConstraints() {
        ...
        // KHÔNG ép width cả VDSRadioButton (label rỗng bên trong sẽ âm width → méo); chỉ ghim height.
    ...
    public func configure(with model: PromotionCardModel) {
        ...
        if let title = model.buttonTitle, !title.isEmpty {
            actionButton.setImage(UIImage.sdk("ic_right_arrow_16", in: .module), for: .normal)
