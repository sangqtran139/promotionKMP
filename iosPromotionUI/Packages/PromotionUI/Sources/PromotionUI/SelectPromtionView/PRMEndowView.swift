...
final public class PRMEndowView: UIView {
    ...
    private var actionTapableView: TapableView!
    private var actionLabel: UILabel!
    /// Shimmer phủ vùng nút "Sử dụng" trong lúc loading (nil khi không loading).
    private var loadingActionShimmer: VDSShimmerView?

    private let maxVisibleCoupons = 2

    /// Theme token host cấu hình (nil = giữ default SDK).
    private var themeToken: VDSDiscountBadgeThemeToken? { VDSThemeRegistry.shared.discountBadge() }
    ...
    /// Tạo 1 mảnh shimmer (VDSShimmerView + nền xám tokenDark05) — pattern chung của SDK.
    private func makeShimmerPiece(corner: CGFloat) -> VDSShimmerView {
        let piece = VDSShimmerView()
