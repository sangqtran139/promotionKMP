 ...
 * test**: Android ba biến trong `PRMOfferWidget`, iOS hai biến trong `PromotionSDKImpl.render`.
 */
class OfferWidgetHostNotifierTest {
    ...
    fun golden_appliedTransition_phatDungMotSuKien() {
        val n = OfferWidgetHostNotifier()

        assertEquals(
            listOf(OfferWidgetHostEvent.VoucherApplied("v1")),
            n.onState(OfferWidgetState(hasLoadedInitial = true, appliedDiscounts = listOf(applied("v1")))),
        )
    }
    ...
    fun golden_renderLaiCungState_khongPhatGiThem() {
        val n = OfferWidgetHostNotifier()
        val state = OfferWidgetState(hasLoadedInitial = true, appliedDiscounts = listOf(applied("v1")))
        n.onState(state)
    ...
    fun golden_chuoiTransitionMotPhien_dungThuTuVaDungSoLan() {
        val n = OfferWidgetHostNotifier()
        val empty = OfferWidgetState(hasLoadedInitial = true)
        val v1 = OfferWidgetState(hasLoadedInitial = true, appliedDiscounts = listOf(applied("v1")))
        val v2 = OfferWidgetState(hasLoadedInitial = true, appliedDiscounts = listOf(applied("v2")))

        val phat = listOf(empty, v1, v1, empty, v2, v2).flatMap { n.onState(it) }

        assertEquals(
            listOf(
                OfferWidgetHostEvent.VoucherApplied("v1"),
                OfferWidgetHostEvent.VoucherApplied("v2"),
            ),
            phat,
        )
    }
    ...
    fun golden_nhipTrungGianThieuId_khongLamPhatHaiLan() {
        val n = OfferWidgetHostNotifier()
        val khongId = OfferWidgetState(hasLoadedInitial = true, totalVoucherCount = 3)
        val coId = OfferWidgetState(hasLoadedInitial = true, appliedDiscounts = listOf(applied("v1")))

        val phat = listOf(khongId, coId, coId).flatMap { n.onState(it) }

        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), phat)
    }
    ...
    fun applied_firesOnceOnTransitionIntoApplied() {
        val n = OfferWidgetHostNotifier()
        n.onState(OfferWidgetState(totalVoucherCount = 2))                       // NOT_APPLIED
        val st = OfferWidgetState(totalVoucherCount = 2, appliedDiscounts = listOf(applied("v1")))

        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), n.onState(st))
    ...
    fun applied_firesAgainAfterLeavingAppliedState() {
        ...
        val st = OfferWidgetState(totalVoucherCount = 2, appliedDiscounts = listOf(applied("v1")))
        n.onState(st)
        n.onState(OfferWidgetState(totalVoucherCount = 2))                       // huỷ áp → NOT_APPLIED
        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), n.onState(st), "áp lại → bắn lại")
    }
    ...
    fun applied_firesAgainWhenVoucherIdChangesWithoutLeavingApplied() {
        ...
        // này — phải so theo id.
        val n = OfferWidgetHostNotifier()
        val v1 = OfferWidgetState(totalVoucherCount = 2, appliedDiscounts = listOf(applied("v1")))
        val v2 = OfferWidgetState(totalVoucherCount = 2, appliedDiscounts = listOf(applied("v2")))

        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), n.onState(v1))
        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v2")), n.onState(v2), "đổi voucher dù vẫn APPLIED → phải bắn lại")
    ...
    fun unavailable_firesTooSoHostAlwaysGetsTheVoucherId() {
        ...
        val n = OfferWidgetHostNotifier()
        n.onState(OfferWidgetState(totalVoucherCount = 2))                       // NOT_APPLIED
        val unavailable = OfferWidgetState(
            totalVoucherCount = 2,
            appliedDiscounts = listOf(applied("v1")),
            discountUnavailable = true,
        )
        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), n.onState(unavailable))
    ...
    fun unavailable_doesNotRefireWhenSameIdJustTurnsInvalid() {
        ...
        val n = OfferWidgetHostNotifier()
        val appliedState = OfferWidgetState(totalVoucherCount = 2, appliedDiscounts = listOf(applied("v1")))
        val nowInvalid = appliedState.copy(discountUnavailable = true)

        assertEquals(listOf(OfferWidgetHostEvent.VoucherApplied("v1")), n.onState(appliedState))
