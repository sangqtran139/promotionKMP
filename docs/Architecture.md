...
- `data/remote/` — `PromotionApiService` (Retrofit interface), `PromotionRemoteDataSource`, `RetrofitClient`, `ApiInterceptor`, `PromotionApiException`.
...
- `domain/model/` — model nghiệp vụ (`VoucherDetail`, `VoucherSearchResult`, `RedemptionSessionResult`…).
...
- `domain/exception/` — exception và error code nghiệp vụ.
...
5. DTO trả về được map `toVoucherSearchResult()` → domain model.
...
- Domain **không** import `android.*`, `retrofit2.*`, `androidx.room.*`.
- Data map DTO → domain model **trước khi** trả lên Domain (không để DTO rò rỉ lên UI).
