...
- [1. Chạy test](#1-chạy-test)
  - [1.1. Test tầng Swift](#11-test-tầng-swift)
  - [1.2. Luôn kiểm số lượng test, đừng tin "BUILD SUCCESSFUL"](#12-luôn-kiểm-số-lượng-test-đừng-tin-build-successful)
...
```bash
cd iosPromotionSDK
xcodebuild test -project PromotionKit.xcodeproj -scheme PromotionKit \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' CODE_SIGNING_ALLOWED=NO
```

**Một lệnh chạy hết bốn bundle**: `PromotionKitTests` (tầng `Entry/`) + `PRMFoundationTests` +
`PRMDesignKitTests` + `PRMPromotionUITests`. Ba cái sau là test target của package local, được nối
vào scheme `PromotionKit` qua `Testables` — scheme tự sinh của từng package **không** có test action.
...
| Scheme để trong `xcuserdata` | Máy khác clone về là mất. `PromotionKit.xcscheme` nay nằm ở `xcshareddata/xcschemes` (đã commit). |
...
`@testable import PromotionKit` **không** kéo theo type Kotlin. Test nào cần dựng `OfferWidgetState`,
`OfferWidgetHostEvent`… thì phải `import PRMKotlinBridge` tường minh (xem `OfferWidgetHostNotifierEmitTests`).

### 1.2. Luôn kiểm số lượng test, đừng tin "BUILD SUCCESSFUL"
