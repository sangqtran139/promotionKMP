...
> (commit `4b0dc59`) rồi trả về, vì nó làm iOS lệch Android — bên Android theme chưa bao giờ rời
> `ui/theme/`. `entry/` chỉ giữ `PromotionSDK`, `PromotionSDKCallback`, `PromotionConfig`,
> `PromotionSDKOptions` và `api/`.
