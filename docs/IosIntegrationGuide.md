...
- Kéo **một** file `PRMSDK.xcframework` vào project, đặt **Embed & Sign**. Xong. Không CocoaPods,
  không SPM, không cài Kotlin/RxSwift.
- Mọi thứ host chạm đều bắt đầu bằng `Promotion*` (`PRMSDK`, `PRMSDKApi`, `PRMSDKTheme`…).
- `import PromotionSDKUI` là import **duy nhất** host cần.
- Cấu hình một lần bằng `PRMSDK.initialize(options:)`, bơm đơn hàng bằng `updateContext(...)`, nhận
  sự kiện qua `PRMSDKCallback`.

---

## 1. SDK đóng gói thế nào (vì sao host "sạch")

SDK ship dạng **một dynamic framework** đóng trong `PRMSDK.xcframework` (binary là Mach-O
...
| Mục | Giá trị |
|---|---|
| Artifact | `PRMSDK.xcframework` (một file duy nhất) |
...
1. Kéo `PRMSDK.xcframework` vào project navigator.
2. Chọn target host → tab **General** → **Frameworks, Libraries, and Embedded Content**.
3. Đặt `PRMSDK.xcframework` = **Embed & Sign**.
...
print(PRMSDK.isInitialized()) // false — link OK là được
```
...
`PRMSDK` là **singleton tĩnh** — mọi điểm vào là `static`. SDK giữ **một** phiên sống tại một thời điểm.
...
PRMSDK.initialize(
    options: PRMSDKOptions(
        ...
        callback: myCallback            // đối tượng conform PRMSDKCallback (xem §7)
    )
)
```
...
| Việc | API |
|---|---|
| Khởi tạo | `PRMSDK.initialize(options:)` |
| Kiểm tra đã init | `PRMSDK.isInitialized() -> Bool` |
...
PRMSDK.updateContext(
    ...
    orderItems: [PRMOrderItem(skuId: "SKU1", quantity: 1, unitPrice: "500000")]
)
```

SDK đọc lại các giá trị này ở **mỗi** request, nên chỉ cần gọi trước khi mở màn / gọi API. Đọc ngược lại
qua `PRMSDK.currentOrderId / currentOrderValue / currentServiceCode / currentMetaData` và `PRMSDK.session`.
...
> (`[PRMSDK] … bị gọi trước initialize()`). Riêng **`api`** vẫn `preconditionFailure` vì kiểu trả
> về không optional. SDK khởi tạo thường là **bất đồng bộ** (chờ login), nên hãy gác điểm vào bằng
> `PRMSDK.isInitialized()` — nếu không, nút bấm sẽ như "không ăn" và widget sẽ trống.
...
PRMSDK.openPromotionDetail(voucherId: "V123", from: self)

// Widget checkout — gắn vào layout của bạn, tự load dữ liệu
let widget = PRMSDK.createEndowView(from: self, orderId: order.id, orderValue: "500000")
container.addSubview(widget)

// Widget checkout kèm dòng sản phẩm (lấy campaign theo SKU)
let widget2 = PRMSDK.createEndowView(
...
```swift
final class MyPromotionCallback: PRMSDKCallback {
    ...
    func onServiceSelected(selection: PRMServiceSelection) { /* điều hướng tới dịch vụ đã chọn */ }
...
Không dùng UI có sẵn thì gọi trực tiếp `PRMSDK.api` (kiểu `PRMSDKApi`). Closure-based, chạy về
main thread. **Phải `initialize` trước.**

```swift
PRMSDK.api.getVouchers(keyword: "grab", page: 0) { result in
    switch result {
    case .success(let page):  // PRMVoucherPage(vouchers: [PRMVoucher], isLastPage)
...
```swift
PRMSDK.configure(theme: PRMSDKTheme(
...
- Truyền `theme` trong `PRMSDKOptions` lúc init cũng được; để `nil` = SDK tự khôi phục theme đã lưu.
