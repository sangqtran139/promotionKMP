...
| `:networkKit` (KMP) | 🚧 Phase 1, UC6 xong — `HttpClient` factory + header tĩnh/động + token provider + request builder + giải mã JSON generic + `NetworkError` cho lỗi transport, chưa tích hợp vào `promotionLogic`. Xem [common/SharedNetworkKit.md](./common/SharedNetworkKit.md). |
