...
import com.ttcn.prm.databinding.PrmViewOfferWidgetBinding
...
import com.ttcn.promotionsdk.presentation.offerwidget.OfferWidgetHostEvent
import com.ttcn.promotionsdk.presentation.offerwidget.OfferWidgetHostNotifier
...
class PRMOfferWidget @JvmOverloads constructor(
    context: Context,
    ...
    private val binding: PrmViewOfferWidgetBinding =
        PrmViewOfferWidgetBinding.inflate(
            LayoutInflater.from(
                ContextThemeWrapper(PRMLocale.wrap(context), R.style.PRMForceLight),
            ),
            this,
            true,
        )
    ...
    private var viewModel: OfferWidgetViewModel? = null

    // ─── Internal UI state ────────────────────────────────────────────────────

    private var currentState: OfferWidgetDisplayState = OfferWidgetDisplayState.NOT_APPLIED
     ...
     *
     * Một instance mỗi widget: nó có trạng thái, hai `PRMOfferWidget` cùng màn phải đếm riêng.
     */
    private val hostNotifier = OfferWidgetHostNotifier()
    ...
    // (`ChoosePromotionIntent.SeedOnce`). Dữ liệu vẫn còn ở `OfferWidgetState` — nơi widget cất kết quả nạp
    ...
    /**
     * Widget vốn KHÔNG có nền: `prm_view_offer_widget.xml` không khai `android:background` nào. Trên host
     ...
     * ép cả subtree gồm cả nền, nên `PRMOfferWidget.swift` không dính.
     *
    ...
    override fun onAttachedToWindow() {
        ...
        //
        // Đây là chỗ sửa lỗi cũ: bản trước `OfferWidgetViewModel.create(viewScope)` nên store chết ở
        ...
        // iOS không dính vì `offerWidgetVM` là property của `PromotionSDKImpl`.
        //
        ...
        val vm = ViewModelProvider(owner, promotionViewModelFactory())[OfferWidgetViewModel::class.java]
        viewModel = vm
        ...
        binding.shimmerOffer.startShimmer()
     ...
     */
    private fun applyFeatureFlag(enabled: Boolean, vm: OfferWidgetViewModel) {
        isVisible = enabled
    ...
    /**
     * Nhận offers user chọn từ màn "Chọn ưu đãi" → [OfferWidgetStore] validate & áp (dùng chung iOS).
     * `internal`: [EligibleOffer] thuộc `promotionLogic`; host dùng qua [ChoosePromotionFragment.forOfferWidget].
     *
     * [onSettled] gọi **đúng một lần ở mọi nhánh** khi lượt validate ngã ngũ, kèm
     * [OfferWidgetApplyOutcome] — màn chọn dựa vào đó để đóng màn, disable ưu đãi, hay báo lỗi. Đối ứng
     * completion của `offerWidgetVM.validateAndApply` bên iOS.
     *
    ...
    internal fun applySelectedOffers(
        ...
        onSettled: ((outcome: OfferWidgetApplyOutcome) -> Unit)? = null,
    ) {
        val vm = viewModel
        if (vm == null) {
            ...
            onSettled?.invoke(OfferWidgetApplyOutcome.Failed(ErrorCodes.GENERAL))
            return
        }
    ...
    fun getCurrentState(): OfferWidgetDisplayState = currentState
     ...
     *
     * Nghiệp vụ nằm ở `OfferWidgetStore.confirmRedemption` (dùng chung với iOS). Trước đây phải qua
    ...
    fun confirmRedemption(
        ...
        scope.launch {
            when (val result = vm.confirmRedemption()) {
                is OfferWidgetConfirmResult.Success -> onSuccess()
                is OfferWidgetConfirmResult.Failure -> {
                    ...
                    //
                    // Cờ tắt vẫn KHÔNG gọi API (`OfferWidgetStore.confirmRedemption` chặn trước) và vẫn trả
    ...
    private fun renderState(state: OfferWidgetState) {
        // Handle error
        state.errorCode?.let {
            ...
            // Xoá được ngay: nơi gọi `validateAndApply` nhận `OfferWidgetApplyOutcome` trả về trực tiếp,
        ...
        binding.shimmerOffer.stopShimmer()
        binding.shimmerOffer.visibility = GONE

        // Trạng thái widget do store quyết định (OfferWidgetStore.widgetState) — View chỉ render.
        when (state.widgetState) {
            OfferWidgetDisplayState.UNAVAILABLE -> {
                currentState = OfferWidgetDisplayState.UNAVAILABLE
                showUnavailableState(state.appliedDiscounts)
            }

            OfferWidgetDisplayState.APPLIED -> {
                currentState = OfferWidgetDisplayState.APPLIED
                showAppliedState(state.appliedDiscounts)
            }

            OfferWidgetDisplayState.NOT_APPLIED -> {
                currentState = OfferWidgetDisplayState.NOT_APPLIED
                showNotAppliedState(state.totalVoucherCount)
            }

            OfferWidgetDisplayState.EMPTY -> {
                currentState = OfferWidgetDisplayState.EMPTY
                showEmptyState()
            }
        }
     ...
     * hay không nằm ở [OfferWidgetHostNotifier] (dùng chung iOS); ở đây chỉ map sự kiện → callback.
     * Chỉ còn `onVoucherApplied`: các callback đếm voucher / bật-tắt / đóng màn đã bỏ, host không cần biết.
     */
    private fun notifyHost(state: OfferWidgetState) = emit(hostNotifier.onState(state))

    /** Map [OfferWidgetHostEvent] dùng chung → callback public của Android. */
    private fun emit(events: List<OfferWidgetHostEvent>) {
        val callback = PromotionSDK.getCallback() ?: return
        events.forEach { event ->
            when (event) {
                is OfferWidgetHostEvent.VoucherApplied -> callback.onVoucherApplied(event.voucherId)
            }
        }
    }
    ...
    private fun setupClickListeners() {
        binding.txtStatusOffer.setOnClickListener {
            when (currentState) {
                OfferWidgetDisplayState.NOT_APPLIED, OfferWidgetDisplayState.UNAVAILABLE -> openChoosePromotionScreen()
                OfferWidgetDisplayState.APPLIED -> {
                    viewModel?.clearApplied()
                }
                OfferWidgetDisplayState.EMPTY -> Unit
            }
        }
    }
     ...
     * [PRMOfferWidget] vào layout (xem [PromotionSDK.openChoosePromotion]).
     *
    ...
    private fun showNotAppliedState(count: Int) {
        binding.apply {
            txtNumberOffer.text = when (count) {
                1 -> root.context.getString(R.string.prm_one_offer)
                else -> root.context.getString(R.string.prm_multiple_offer, count)
            }
            txtNumberOffer.visibility = VISIBLE
            rcvOffer.visibility = GONE
            txtStatusOffer.visibility = VISIBLE
            txtStatusOffer.text = root.context.getString(R.string.prm_use_voucher)
        }
        applyTokenInternal(lastAppliedToken ?: PromotionThemeRegistry.discountBadgeToken())
    }
    ...
    private fun showAppliedState(details: List<AppliedDiscount>) {
        binding.apply {
            ...
            txtNumberOffer.visibility = GONE
            rcvOffer.visibility = VISIBLE
            txtStatusOffer.visibility = VISIBLE
            txtStatusOffer.text = root.context.getString(R.string.prm_cancel_voucher)
        }
        applyTokenInternal(lastAppliedToken ?: PromotionThemeRegistry.discountBadgeToken())
    }
    ...
    private fun showUnavailableState(details: List<AppliedDiscount>) {
        binding.apply {
            ...
            rcvOffer.visibility = VISIBLE
            txtStatusOffer.visibility = VISIBLE
            txtStatusOffer.text = root.context.getString(R.string.prm_change_voucher)
        }
        applyTokenInternal(lastAppliedToken ?: PromotionThemeRegistry.discountBadgeToken())
    }
    ...
    private fun showEmptyState() {
        binding.apply {
            txtNumberOffer.text = root.context.getString(R.string.prm_no_offer)
            txtNumberOffer.visibility = VISIBLE
            rcvOffer.visibility = GONE
            txtStatusOffer.visibility = INVISIBLE
        }
        applyTokenInternal(lastAppliedToken ?: PromotionThemeRegistry.discountBadgeToken())
    }
    ...
    private fun applyTokenInternal(token: PRMDiscountBadgeToken?) {
        ...
        token?.actionTextColor?.let { binding.txtStatusOffer.applyTextColorIfSet(it) }
