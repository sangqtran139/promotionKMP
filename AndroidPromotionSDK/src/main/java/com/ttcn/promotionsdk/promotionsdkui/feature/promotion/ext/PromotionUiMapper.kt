package com.ttcn.promotionsdk.promotionsdkui.feature.promotion.ext
 ...
 *
 *  1. Dựng **domain request** từ model UI / [PRMAppliedDiscount].
 *  2. Diễn giải **kết quả validate → [PRMAppliedDiscount]** per-offer qua `ValidateDiscountsResult`
 *     ([appliedDiscountFor]) cho public surface (callback, [PRMEndowView]).
 *  3. Map **domain model ([EligibleOffer], [AvailableService]) → model UI**.
 *
 * [PRMAppliedDiscount] là model **public** của SDK (ui/entry); domain (use case, repository)
...
internal fun List<PRMAppliedDiscount>.toValidateDiscountsRequest(
    orderId: String,
    orderValue: String,
): ValidateDiscountsRequest = ValidateDiscountsRequest(
    orderId = orderId,
...
internal fun List<PRMAppliedDiscount>.toCreateRedemptionRequest(
    orderId: String,
    orderValue: String,
...
// ─── Map domain result → PRMAppliedDiscount (model public) ──────────────────────

/**
 * Dựng [PRMAppliedDiscount] cho offer [objectId]/[objectType] từ kết quả validate, đi qua đúng hai helper
 ...
 */
internal fun ValidateDiscountsResult.appliedDiscountFor(objectId: String, objectType: String): PRMAppliedDiscount =
    PRMAppliedDiscount(
        objectId = objectId,
