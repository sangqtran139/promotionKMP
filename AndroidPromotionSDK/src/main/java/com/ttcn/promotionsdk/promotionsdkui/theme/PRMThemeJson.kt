...
/**
 * Serialize [PRMSDKTheme] ↔ JSON. **Một định dạng duy nhất, dùng chung Android và iOS** —
 * đối ứng phần `ThemeDTO` trong `PRMSDKTheme.swift`. Đối tác ship một file theme cho cả hai.
 *
 ...
 * iOS, dù comment bên Swift ghi "mirror Android PRMThemeJson". Đưa JSON bên này sang bên kia
...
object PRMThemeJson {
    ...
    fun toJson(theme: PRMSDKTheme): String = gson.toJson(theme.toDto())

    fun fromJson(json: String): PRMSDKTheme? =
...
private fun PRMSDKTheme.toDto() = ThemeDto(
...
private fun ThemeDto.toTheme() = PRMSDKTheme(
